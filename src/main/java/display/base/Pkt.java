...
public class Pkt {
    ...
    public Pkt(Point p_ptZiel, Color p_clColor, Dimension p_dimSize) {
        m_ptZiel = p_ptZiel;
        // default Geschwindigkeit
        m_dbV = DB_V;
        m_dmSize = p_dimSize;
    ...
    public Pkt(Point p_ptPos, Point p_ptZiel, Color p_clColor) {
        ...
        // Berechnung des Winkels m_dbWinkel zwischen Verbindungslinie von
        // p_ptPos
        // und p_ptZiel und der Horizontalen
        m_dbWinkel = Math.atan((p_ptPos.y * 1.0 - p_ptZiel.y * 1.0) / (p_ptPos.x * 1.0 - p_ptZiel.x * 1.0));
    }

    public Pkt(Point p_ptPos, Point p_ptZiel, double p_dbV, Dimension p_dmSize, Color p_clColor) {
        m_dbPos[0] = p_ptPos.x;
        m_dbPos[1] = p_ptPos.y;
        m_bPaintStatus = true;
        m_ptZiel = p_ptZiel;
    ...
    public Point getPos() {
        return new Point(new Long(Math.round(m_dbPos[0])).intValue(), new Long(Math.round(m_dbPos[1])).intValue());
    }
    ...
    public boolean nextPosition() {
        ...
        /*
         * DB_XTOLERANCE+=0.00001; DB_YTOLERANCE+=0.00001;
         */
        return isReady();
    }
    ...
    public void paint(Graphics p_G) {
        if (getPaintStatus()) {
            ...
            p_G.fillRoundRect(new Long(Math.round(m_dbPos[0])).intValue() + 1, new Long(Math.round(m_dbPos[1])).intValue() + 1, m_dmSize.width - 1, m_dmSize.height - 1, 0, 0);
        }
    }
